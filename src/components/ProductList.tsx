...
import { deleteProduct, searchProducts } from "@/app/actions";
import {
    Search,
    Plus,
    Printer,
...
export default function ProductList({
    ...
    // Client-side filter for the current page's products
    ...
    const handleSearch = useCallback((value: string) => {
        ...
        searchTimeoutRef.current = setTimeout(async () => {
            setIsLoading(true);
            try {
                const result = await searchProducts(value || undefined, 1, 50);
    ...
    const selectAll = () => {
        if (selectedProducts.size === filteredProducts.length) {
            ...
            setSelectedProducts(
                new Set(filteredProducts.map((p) => p._id as string)),
            );
        }
    };
    ...
    const handleBatchDelete = async () => {
        if (
            confirm(
                `คุณต้องการลบสินค้าที่เลือก ${selectedProducts.size} รายการหรือไม่?`,
            )
        ) {
                        ...
                        <div className="flex items-center justify-between gap-3 pt-12 md:pt-0">
                            ...
                            <div className="hidden md:flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    className="btn btn-primary gap-2 shadow-lg shadow-primary/25"
                                >
                    ...
                    <div className="sticky top-[72px] md:top-[80px] z-10 bg-primary/10 border-b border-primary/20 px-4 md:px-6 py-3 flex items-center justify-between backdrop-blur-md supports-[backdrop-filter]:bg-primary/5">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="btn btn-danger gap-1.5 text-xs md:text-sm px-3 md:px-4"
                            >
                                            ...
                                            <div className="flex justify-between items-start mb-4 pr-8">
                                                <div>
                                                    <div className="flex items-center gap-2 mb-1">
                                                        ...
                                                        {product.category && (
                                                            <span
                                                                className={`text-[10px] px-2 py-0.5 rounded-full font-medium border ${getCategoryColor(product.category)}`}
                                                            >
                                                                {
                                                                    product.category
                                                                }
                  ...
                  `}
                                >
                                    <input
                                        ...
                                        onChange={() =>
                                            toggleSelect(product._id as string)
                                        }
                            ...
                            <div className="space-y-3">
                                {Array.from(selectedProducts).map((id) => {
                                    ...
                                    return (
                                        <div
                                            key={id}
                                            className="bg-card p-3 rounded-lg border border-border flex items-center justify-between"
                                        >
